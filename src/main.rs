...
impl NShape<u32> for Square<u32> {
    fn new(value: u32) -> Square<u32> {
        Square::<u32> {
            area_value: value.pow(2) as f64,
            side: value,
        }
    }
}
...
impl NShape<f64> for Square<f64> {
    fn new(value: f64) -> Square<f64> {
        Square::<f64> {
            area_value: value.powi(2),
            side: value,
        }
    }
}
...
impl NShape<String> for Square<String> {
    fn new(value: String) -> Square<String> {
        Square::<String> {
            area_value: value.parse::<i32>().unwrap().pow(2) as f64,
            side: value,
        }
    }
}
...
impl AsNShape<f64> for Triangle<f64> {
    fn new(value: f64, value2: f64) -> Triangle {
        Triangle {
            area_value: value * value2 / 2.0,
            base: value,
        }
    }
}
...
impl<S> TriShape<S, f64> for Pyramid<S, f64>
where
    S: Shape,
{
    fn new(value: S, value2: f64) -> Pyramid<S, f64> {
        Pyramid::<S, f64> {
            base: value,
            height: value2,
        }
